...
enum ComputerUsePlannerClient {
    ...
    - Treat latest_window_state.process_id as the executable target process when a tool accepts it. Mutating tools do not accept window_id; use get_window_state with window_id to refresh that specific window when possible before acting from the fresh state.
    ...
    - If latest_window_state.target_mismatch is present, the requested window was not refreshed. Do not use current-window mutating tools from that state; re-orient with list_windows/get_window_state for the intended or actual visible window before acting.
    ...
    - For text entry, prefer target-scoped calls: include process_id, and include element_index/element_id when an editable target or web editor surface is visible in the latest state. Mutating tools do not use window_id as an execution guard; use get_window_state with window_id to re-orient, then act from the fresh screenshot/AX context.
